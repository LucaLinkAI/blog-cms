...
function renderBlock(block: Record<string, unknown>, depth = 0): string {
  ...
  switch (type) {
    ...
    case "video": {
      ...
      const caption = escapeHtml((props.caption as string) ?? "");
      if (!url) return "";
      return `<figure><video src="${url}" controls class="rounded-lg max-w-full w-full"></video>${caption ? `<figcaption class="text-center text-sm text-muted-foreground mt-2">${caption}</figcaption>` : ""}</figure>`;
    }
    ...
    case "audio": {
      ...
      const caption = escapeHtml((props.caption as string) ?? "");
      if (!url) return "";
      return `<figure><audio src="${url}" controls class="w-full"></audio>${caption ? `<figcaption class="text-center text-sm text-muted-foreground mt-2">${caption}</figcaption>` : ""}</figure>`;
    }

    case "file": {
      const url = escapeHtml((props.url as string) ?? "");
      const name = escapeHtml((props.name as string) ?? "Download file");
      const caption = escapeHtml((props.caption as string) ?? "");
      if (!url) return "";
      return `<figure><a href="${url}" download class="inline-flex items-center gap-2 rounded-lg border border-border px-4 py-3 text-sm font-medium hover:bg-muted transition-colors no-underline"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/><line x1="12" y1="18" x2="12" y2="12"/><line x1="9" y1="15" x2="12" y2="18"/><line x1="15" y1="15" x2="12" y2="18"/></svg>${name}</a>${caption ? `<figcaption class="text-sm text-muted-foreground mt-2">${caption}</figcaption>` : ""}</figure>`;
    }
